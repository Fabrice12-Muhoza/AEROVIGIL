...
export const paymentAPI = {
  ...
  checkPaymentStatus: (paymentId) => api.get(`/payments/status/${paymentId}`),
    ...
    // Get payment methods
    getSupportedPaymentMethods: (countryCode = 'RW') => 
        api.get(`/payments/methods?countryCode=${countryCode}`),
    ...
    // Standalone payment status
    getTransactionStatus: (paymentId) =>
        api.get(`/payments/transaction/${paymentId}`),
    // Mock webhook for testing
  mockWebhook: (data) => api.post('/payments/mock-webhook', data)
};
